...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  const productUrl = `/products/${product.id}-${product.name
    .toLowerCase()
    .replace(/\s+/g, "-")}`;
    ...
    <>
      ...
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            offers: {
              "@type": "Offer",
              url: `${process.env.NEXT_PUBLIC_APP_URL}${productUrl}`,
              priceCurrency: "USD",
              price: product.price,
      ...
      <Card className="group h-full flex flex-col overflow-hidden transition-all duration-300 hover:shadow-xl hover:scale-[1.02] border-border/50 hover:border-border bg-card/50 backdrop-blur-sm">
        ...
        <CardHeader className="p-0 relative">
          <Link
            href={`${productUrl}?color=${encodeURIComponent(
              productTypes.color
          ...
          >
            <div className="relative aspect-[3/4] overflow-hidden">
              <Image
                ...
                alt={`${product.name} in ${productTypes.color}`}
                fill
        ...
        <CardContent className="flex-1 flex flex-col gap-3 sm:gap-4 p-3 sm:p-4 lg:p-6">
          <h2 className="font-semibold text-base sm:text-lg leading-tight line-clamp-2 group-hover:text-primary transition-colors duration-300">
            {product.name}
          </h2>
          ...
          <div className="space-y-3 sm:space-y-4">
            {/* Size Selection (shadcn/ui Select) */}
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleProductTypeChange({ type: "size", value })
                }
              >
